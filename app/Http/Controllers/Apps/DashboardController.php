...
class DashboardController extends Controller
{
    ...
    public function __invoke(Request $request)
    {
        ...
        $chart_sales_week = DB::table('transactions')->addSelect(DB::raw("DATE(created_at) as date, SUM(grand_total) as grand_total"))->where("created_at", ">=", $week)->groupBy('date')->get();
        ...
        $chart_best_product = DB::table('transaction_details')->addSelect(DB::raw("products.title as title, SUM(transaction_details.qty) as total"))->join('products', 'products.id', '=', 'transaction_details.product_id')->groupBy('transaction_details.product_id')->orderBy('total', 'DESC')->limit(5)->get();

        if (count($chart_best_product)) {
            foreach ($chart_best_product as $result) {
